...
\paragraph{La méthode polynomiale}
...
\subparagraph{Lagrange}
\vspace{0.5cm}
\dots
\subparagraph{Hermite}
\cite{bajaj}
\vspace{0.5cm}
\dots
\subparagraph{Splines}
...
\subparagraph{Cubic Interpolation}
\cite{tanaka}
\vspace{0.5cm}


\paragraph{Les méthodes à base de distance}
\paragraph{IDW}
\vspace{0.5cm}
...
\paragraph{Méthodes géostatiques}
...
Pour l'instant, je n'ai démontré que par l'expérimentation que cette méthode était linéaire. Le premier test étais de créer une variable linéaire et de faire une interpolation sur un second maillage. En comparant la valeur analytique et numérique, l'erreur observée est nulle. Ensuite j'ai essayé de tracer le log de l'erreur au sens des moindres carrés en fonction du raffinement du maillage source. Cela a étonnamment montré une convergence d'ordre quatre pour tous types de cellules (alors que nous nous attendions à une convergence d'ordre un pour le 1D) ////Graphiques et tests avec un coefficient (puissance) 2 pour 2D etc. ////
