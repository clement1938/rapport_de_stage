...
\begin{table}[ht]
    ...
    \renewcommand{\arraystretch}{1.5} % Ajuste l'espacement entre les lignes
    \resizebox{\textwidth}{!}{ % Ajuste la largeur du tableau à la largeur du texte
    \begin{tabular}{ |>{\centering\arraybackslash}m{2.5cm}|>{\centering\arraybackslash}m{1.5cm}|>{\centering\arraybackslash}m{1.5cm}|>{\centering\arraybackslash}m{2cm}|>{\centering\arraybackslash}m{3cm}|>{\centering\arraybackslash}m{2.5cm}| }
        \hline
        Maillage &  MPI & Sortie de la solution & Équation parabolique (ie. flux visqueux) & Taille du domaine limité en $2^n$ \\
        \hline
        \textbf{TreeMesh}  oui  & oui (limité à 10 rang) & oui & oui (sauf en MPI) & oui \\
        \hline
        \textbf{TreeMesh}  oui & non & oui & oui (sauf en MPI) & oui \\
        \hline
        \textbf{Structured}  oui & non & oui & non & non \\
        \hline
    \end{tabular}
    }
